...
class KDTree
{
  ...
  public:
    ...
    int quickselect(int k, int left, int right, int currDim, vector<Point<Dim>> &newPoints);

    int partition(int left, int right, int currDim, int pivotIndex, vector<Point<Dim>> &list);

    void createTree(KDTreeNode * &node, int beg, int end, int dimC, vector<Point<Dim>> &newPoints);
    /**
    ...
    KDTreeNode * findNearestNeighbor(const Point<Dim>& query, KDTreeNode *closestPoint, KDTreeNode *currentPoint, int dimension) const;
