                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     */
     ...
     if(sing->freq.getFrequency() <= merg->freq.getFrequency()){
       singleQueue.pop();
       return sing;
     }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    TreeNode * ins;
    int fs = frequencies.size();
    for(int i = 0; i<fs; i++){
      ins = new TreeNode(frequencies[i]);
      singleQueue.push(ins);
    }

    TreeNode *sm1, *sm2, *newN;
    int newF;

    while(singleQueue.size()+mergeQueue.size()!=1){
      sm1=removeSmallest(singleQueue, mergeQueue);
      sm2=removeSmallest(singleQueue, mergeQueue);

      newF=sm1->freq.getFrequency()+sm2->freq.getFrequency();
      newN= new TreeNode(newF);
      newN->left=sm1;
      newN->right=sm2;

      mergeQueue.push(newN);
    }
     ...
     */
     ...
     if (bfile.hasBits()) {
       ...
       if(bfile.getNextBit()){
         ...
         created = new TreeNode(temp);
       }

       else{
         created = new TreeNode(0);
