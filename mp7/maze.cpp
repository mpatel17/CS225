...
void SquareMaze::makeMaze(int w, int h)
{
    ...
    //iterate through array and use helper function to delete random walls while preventing cycles
    for(int i = 0; i < width; i++) {
...
vector<int> SquareMaze::solveMaze()
{
    ...
    //use parent vector to backtrack from dest to origin and insert directions into return vector
...
PNG* SquareMaze::drawCreativeMaze() const
{
    ...
    for(unsigned i = 10; i<ret->width(); i++){
        ret->getPixel( i, 0) = purple; //INSERT COLOR OF TOP BORDER
    }

    for(unsigned i = 0; i<ret->height(); i++){
        ret->getPixel( 0, i)= purple;//INSERT COLOR OF LEFT BORDER
    }
    ...
    for(int i = 0; i < width; i++) {
        for(int j = 0; j < height; j++) {
            ...
            if(maze[i][j].right){
                for(int k = 0; k<=10; k++){
                    ret->getPixel( (i+1)*10, j*10 + k)= wall;//INSERT COLOR OF WALL
                }
            }

            if(maze[i][j].down){
                for(int k = 0; k<=10; k++){
                    ret->getPixel( i*10 + k, (j+1)*10) = wall;//INSERT COLOR OF WALL
                }
            }
    ...
    for(unsigned i = 0; i<ret->width(); i++){
        ret->getPixel( i, ret->height()-1) = purple; //INSERT COLOR OF BOTTOM BORDER
    }

    for(unsigned i = 0; i<ret->height(); i++){
        ret->getPixel( ret->width()-1, i)= purple; //INSERT COLOR OF RIGHT BORDER
    }
...
PNG* SquareMaze::drawCreativeMazeWithSolution()
{
    ...
    for(unsigned i = 0; i < path.size(); i++){
        ...
        HSLAPixel red ( rand() % 360 , 1.0 , 0.27 , 1); //CHANGE THIS TO COLOR OF SOLUTION
      counter = 0;
